...
def create_app(secure_client_credential=None):
    ...
    def token_details():
        print(dir(ms_identity_web.id_data))
        print(ms_identity_web.id_data)
        ms_identity_web.acquire_token_silently()
        print(ms_identity_web.id_data._access_token)
        token = f'Bearer {ms_identity_web.id_data._access_token}'
        results = requests.get('https://niblscoin.b2clogin.com/niblscoin.onmicrosoft.com/B2C_1_nibls-webapp-acctmgnt-signupsignin-0/oauth2/v2.0/token', headers={'Authorization': token}).json()
        print(results)
...
if __name__ == '__main__':
    ...
    app.run(ssl_context='adhoc',port=5001) # create an adhoc ssl cert for HTTPS on 127.0.0.1
